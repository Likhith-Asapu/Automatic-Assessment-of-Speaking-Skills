...
def train_svm_classifier_for_valence():
    ...
    def create_training_data(example):
        ...
        if example['emotion'] == 'neu' or example['emotion'] == 'hap' or example['emotion'] == 'sad' or example['emotion'] == 'ang':
            ...
            if example['valence'] < 2.5:
              y.append(0)
            elif example['valence'] < 4:
              y.append(1)
            else:
              y.append(2)
    dataset.map(
...
X,y = train_svm_classifier_for_valence()
...
for batch in train_loader:
    ...
    loss.backward()
    optimizer.step()
torch.save(model,'bilstm_text_valence.pt')
